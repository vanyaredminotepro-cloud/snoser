...
- Команды `/start /status /pause /resume /write_news /anketa /emoji_reload /emoji_list`.
- Логирование в:
  - console
  - `logs/bot.log`
  - `logs/errors.log`
...
- `/anketa` — меню анкет: известный человек / группировка / страна (отправка админу в ЛС).
- `/admin` — инлайн админ-панель (только для администратора).
